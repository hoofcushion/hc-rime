...
local function numberSep (partDecimal,is_money)
 return
  partDecimal and
  (is_money and "元" or "点") or
  (is_money and "元整" or "")
end
...
local translator <const> =
{
 ...
 func=function(input,seg,env)
  ...
  if code=="" then
   Utils.tipsCtx(env,"〔请输入数字〕",true)
   return
  elseif not (code:find("^%d+$") or code:find("^%d-%.%d*$")) then
   Utils.tipsCtx(env,"〔数字不合法〕",true)
   return
  elseif #code:match("^(%d+)")>MAX_MAG then
   Utils.tipsCtx(env,"〔位数超过限制〕",true)
   return
  end
  Utils.tipsCtx(env,"〔大写数字〕",true)
  for i=1,4 do
