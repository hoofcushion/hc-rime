...
M.preset_color_schemes={
 ...
 default_dark={
  ...
  border_color                =c("#a03575"),
  candidate_text_color        =c("#b2b2b2"),
  comment_text_color          =c("#e34ca8"),
  hilited_candidate_back_color=c("#a03575"),
  hilited_comment_text_color  =c("#ff6ff5"),
  hilited_label_color         =c("#000000"),
  hilited_text_color          =c("#ffffff"),
  label_color                 =c("#b2b2b2"),
 },
}
---@class hoofcushion.trime.preset_keys
M.preset_keys={
 Shift_L                    ={label="上档",send="Shift_L",shift_lock="ascii_long"},
 Return                     ={label="enter_labels",send="Return"},
 Return2                    ={label="回车",send="Return"},
 Escape                     ={label="取消",send="Escape"},
 BackSpace                  ={repeatable=true,label="退格",send="BackSpace"},
 Delete                     ={repeatable=true,label="刪除",send="Delete"},
 Home                       ={repeatable=true,label="行首",send="Home"},
 End                        ={repeatable=true,label="行尾",send="End"},
 Page_Up                    ={repeatable=true,label="上页",send="Page_Up"},
 Page_Down                  ={repeatable=true,label="下页",send="Page_Down"},
 ...
 Shape_switch               ={toggle="full_shape",send="Mode_switch",states={"半角","全角"}},
 Simp_switch                ={toggle="simplification",send="Mode_switch",states={"汉字","漢字"}},
 Punct_switch               ={toggle="ascii_punct",send="Mode_switch",states={"。，","．，"}},
 IME_switch                 ={label="语言",send="LANGUAGE_SWITCH"},
 ...
 Voice                      ={label="语音",command="run",option="android.speech.action.RECOGNIZE_SPEECH"},
 ...
 Keyboard_back              ={label="返回",send="Eisu_toggle",select=".default"},
 ...
 Redo                       ={label="重做",send="Control+Shift+z"},
 Remove_Left                ={label="左清",text="{Shift+Home}{Delete}"},
...
local keys={
 ...
 key_10=merge({click="10"},{swipe_left=K.asciitilde,swipe_right=K.asciitilde,long_click=K.asciitilde,swipe_up=K.asciitilde,swipe_down=K.asciitilde},offset_config.upper_right),
 ...
 key_s=merge({click="s"},{swipe_left="Home",swipe_right="End",label_symbol="↞   ↠"}),
 key_d=merge({click="d"}),
 key_f=merge({click="f"}),
 ...
 key_shift=merge({click=K.Shift_L},{width=15}),
 ...
 key_backspace_2=merge({click=K.BackSpace},{swipe_left=K.Remove_Left,swipe_right=K.Remove_Left,swipe_up=K.Remove_Left,swipe_down=K.Remove_Left,label_symbol="左清"},{width=15}),
 key_keyboard_number=merge({click=K.Keyboard_number},{long_click=K.liquid_keyboard_clipboard},{width=20}),
 key_keyboard_back=merge({click=K.Keyboard_back},{long_click=K.liquid_keyboard_clipboard},{width=20}),
 key_comma=merge({click=","},{swipe_left="!",swipe_right="!",long_click="!",swipe_up="!",swipe_down="!",label_symbol="!"},{width=15}),
 key_space=merge({click=K.space},{long_click=K.Mode_switch,swipe_up=K.Mode_switch,swipe_left=K.Left,swipe_right=K.Right},{width=30}),
 key_period=merge({click="."},{swipe_left="?",swipe_right="?",long_click="?",swipe_up="?",swipe_down="?",label_symbol="?"},{width=15}),
 key_return=merge({click=K.Return},{long_click=K.Menu,swipe_up=K.Theme_settings,swipe_left=K.Color_settings},{width=20}),
}
M.preset_keyboards={}
defer(function()
 ...
 for name,keyboard in pairs(M.preset_keyboards) do
  if keyboard.offset then
   merge(keyboard,keyboard.offset)
   keyboard.offset=nil
  end
...
M.preset_keyboards.default={
 ...
 keys={
  ...
  keys.key_k,
  keys.key_l,
  merge({},keys.key_semicolon,{width=7.5}),
  keys.key_shift,
  keys.key_z,
  keys.key_x,
  keys.key_c,
  ...
  keys.key_n,
  keys.key_m,
  keys.key_backspace_2,
  keys.key_keyboard_number,
  keys.key_comma,
  keys.key_space,
  keys.key_period,
  keys.key_return,
...
M.preset_keyboards.number={
 ...
 keys={
  ...
  merge({click="%"}),
  merge({click="^"}),
  merge({click=1},{width=20}),
  merge({click=2},{width=20}),
  merge({click=3},{width=20}),
  merge({click="+"}),
  merge({click="-"}),
  merge({click="!"}),
  merge({click="|"}),
  merge({click=4},{width=20}),
  merge({click=5},{width=20}),
  merge({click=6},{width=20}),
  merge({click="*"}),
  merge({click="/"}),
  merge({click="("}),
  merge({click=")"}),
  merge({click=7},{width=20}),
  merge({click=8},{width=20}),
  merge({click=9},{width=10}),
  merge({click=0},{width=10}),
  merge({click="~"}),
  merge({click="="}),
  keys.key_keyboard_back,
  keys.key_comma,
  keys.key_space,
  keys.key_period,
  keys.key_return,
